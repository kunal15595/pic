...
//	-----------------------------------------------------------------------------------------------
//	FILENAME:			tutorial06_EventFlags.c
//	-----------------------------------------------------------------------------------------------
//	Description:
//  ------------
//	Tutorial application 6: Two threads both in an endless loop waiting for a timer to expire. The 
...
//	as thread2 because of the difference in timer period. Connected to each of the timers is an
//	event group flag. This event group belongs to a third thread. As a result this third thread 
//	when either of the timers expire. This third thread will update another global watch variable. 
//	Because the third thread runs on every timer expiration, the count value of this third thread
//	will be the sum of the count values of the first two threads.
//	===============================================================================================
//	INCLUDES:
//	===============================================================================================
//
...
//	-----------------------------------------------------------------------------------------------
TAVIX_THREAD_REGULAR t06_thread1(void* p);
TAVIX_THREAD_REGULAR t06_thread2(void* p);
TAVIX_THREAD_REGULAR t06_thread3(void* p);
...
//	===============================================================================================
//	FUNCTION:	t06EventFlags
//	=========================
//	Main function for d06EventFlags. This function is called from avixMain present in 
//	_tutorialMain.c and is responsible for initializing the application.
//	===============================================================================================
//
void t06EventFlags(void)
{
	//---------------------------------------------------------------------------------------------
	...
	//---------------------------------------------------------------------------------------------
	avixThread_Create("THR1", t06_thread1, NULL, 1, 500, AVIX_THREAD_READY);
	avixThread_Create("THR2", t06_thread2, NULL, 1, 500, AVIX_THREAD_READY);


	//---------------------------------------------------------------------------------------------
	// Register a thread that will wait for the other threads to set flags.
	//---------------------------------------------------------------------------------------------
	avixThread_Create("THR3", t06_thread3, NULL, 2, 500, AVIX_THREAD_READY);
}



//	===============================================================================================
//	FUNCTION:	t06_thread1
//	=======================
...
TAVIX_THREAD_REGULAR t06_thread1(void* p)
{
	...
	//---------------------------------------------------------------------------------------------
	// The id of the thread is connected to the timer so when the timer expires it will 
	// automatically set the flag and this is no longer required by the thread code to be done.
	//---------------------------------------------------------------------------------------------
	avixTimer_ConnectEventGroup
		(
			timerId,				// When timer 'timerId' expires
			flagThread.asEventId,	// in the event group of thread 'flagThread'
			AVIX_EVENT_GROUP_SET,	// set is
			AVIX_EF(0)				// flag 0 without the thread needing to do this
		);
...
TAVIX_THREAD_REGULAR t06_thread2(void* p)
{
	...
	avixTimer_ConnectEventGroup
		(
			...
			flagThread.asEventId,	// in the event group of thread 'flagThread'
			AVIX_EVENT_GROUP_SET,	// set is
			AVIX_EF(1)				// flag 1 without the thread needing to do this
		);
...
TAVIX_THREAD_REGULAR t06_thread3(void* p)
{
	...
	while(1)
	{
		...
		flags = avixEventGroup_Wait
			(	
				avixThread_GetIdCurrent().asEventId,	// Thread id converted to eveng group id
